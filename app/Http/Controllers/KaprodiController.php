...
class KaprodiController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Dapatkan ID dari semua template surat yang ditujukan kepada Kaprodi ini
        $letterTemplateIds = LetterTemplate::where('forward_to', $userId)->pluck('id');
        
        // Mengambil semua template surat untuk dropdown filter
        $letterTemplates = LetterTemplate::all();

        // Query dasar untuk permintaan surat yang ditujukan kepada Kaprodi ini
        $baseQuery = LetterRequests::whereIn('letter_template_id', $letterTemplateIds);

        // Hitung total permintaan tertunda (pending)
        $pendingCount = (clone $baseQuery)->where('status', 'pending')->count();

        // Hitung permintaan yang disetujui hari ini
        $approvedTodayCount = (clone $baseQuery)
            ->where('status', 'approved')
            ->whereDate('updated_at', now()->toDateString())
            ->count();
        
        // Hitung total permintaan bulan ini
        $totalThisMonthCount = (clone $baseQuery)
            ->whereMonth('created_at', now()->month)
            ->whereYear('created_at', now()->year)
            ->count();
            
        // Query untuk tabel permintaan tertunda
        $query = (clone $baseQuery)->where('status', 'pending')
            ->with('user', 'letterTemplate')
            ->orderBy('created_at', 'desc');

        // Menerapkan logika filter (tetap sama)
